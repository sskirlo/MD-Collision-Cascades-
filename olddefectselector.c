...
int findLocOrder(struct listlist *mdata,struct entry *rd1,int size,struct entry *d1,struct entry *dd1,int *count,double *dim,int *adim,int scale,double a0, char *inlist,FILE *output)
{
	int * refids; 
	...
	for(n=0; n<size; n++)
	{
		...
		if(refids[n]==1 ) //check if in ref id list
		{
			...
			if(d1[n].ent[3]==1) //check if Al atom
			{
				...
				if(flag<=6)
					S=S+(nicount/3-3); //calculate local order parameter
				else
				{
...
//finds 12 nearest neighbors and excludes those that are already in list
int findSurface(struct listlist *mdata,struct entry *rd1,int size,struct entry *d1,struct entry *dd1,int *count,double *dim,int *adim,int scale,double a0, char *inlist,FILE *output)
{
	int * refids; 
	int * surfids; 
	...
	surf=0; 
	//(*count)=0; //!!!!!!!!!!need to remove this if want to work properly again !!!!!!!!!!
	for(n=0; n<size; n++)
	{
		...
		else if(surfids[n]>6) //say has to be referenced at least 2 times
		{
			dd1[(*count)]=d1[n]; //add surface atoms to output buffer for visualization
			dd1[(*count)].ent[4]=-10; 
			(*count)++;
		...
		else if(d1[n].ent[4]!=-100) //use this part to fill in rest of lattice so we can visualize contrast in region
		{
			dd1[(*count)]=d1[n]; //add surface atoms to output buffer for visualization
			dd1[(*count)].ent[4]=10; 
			(*count)++; 
...
int findKECluster(struct listlist *data,struct entry *rd1,struct entry *d1,int size,struct entry * dd1,int * count,double a0,int scale,double *dim,int *adim, double cutoff,double stepsize,int prevcount)
{
	FILE *fp; 
	struct entry neighs[50];
	int pos[3];  
	...
	int max=5500; 
	int min=5000; 
	int mincount=8; //id has to come up at least twice in neighbor lists, use to make regions better connected
	int range=2; 
	...
	//after find backbone atoms, search neighbors to get full list of clusrer
	for(n=0; n<(*count); n++)
	{
		leng=0; 
		getmatpos(pos,dd1[n],adim,dim); //get position
	...
	else //do recursive call and change kenetic energy cutoff up or down to get ideal size
	{
		...
		if(ccount<=min)
			cutoff=cutoff-stepsize; 
		findKECluster(data,rd1,d1,size,dd1,count,a0,scale,dim,adim,cutoff,stepsize,ccount); 
		return 0; 
	} 	
}
...
int calculateRMS(struct listlist *mdata, struct entry *rd1, struct entry *d1,int size, struct entry * dd1, int *count, double a0, int scale, double *dim,int *adim)
{
	...
	printing=0; 
	FILE *fp; 
	fp=fopen("antisite.list","r"); 
	if(1==1)
	{
		printf("no previous antisite list, printing out \n");
		printing=1;  
	}
	if(printing==1)
	{
...
int main(int argc, char* argv[])
{
	...
	while((fgets(text[99],buff1,master))!=NULL)
	{
		...
		*(text[98]+n-6)='\0';
		...
		if(strcmp(defect_type,kecluster )==0)
		{
			findKECluster(mdata,rd1,d1,atoms,dd1,&count,a0,scale,dim,adim,cutoff,0.1,0);
			printf("finding surface area of defect cluster \n"); 
			count=0; 
			findSurface(mdata,rd1,atoms,d1,dd1,&count,dim,adim,scale,a0,"antisite.list",keclusterfile);  
		}
		if(strcmp(defect_type,findCascades)==0)
		{
		...
		{
			printf("finding surface area of defect cluster \n"); 
			findSurface(mdata,rd1,atoms,d1,dd1,&count,dim,adim,scale,a0,listfile,NULL); 
		}
		if(strcmp(defect_type,locOrder)==0)
		{
			printf("finding local order parameter in volume region \n"); 
			findLocOrder(mdata,rd1,atoms,d1,dd1,&count,dim,adim,scale,a0,listfile,NULL); 
		}
		if(strcmp(defect_type,dispsCount)==0)
		{
